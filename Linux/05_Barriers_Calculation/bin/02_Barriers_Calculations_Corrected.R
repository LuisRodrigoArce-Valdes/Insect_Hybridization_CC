...
# Plotting
png("../figures/03_Odonata_Scatterplot_1D.png", width = 18, height = 10, units = "cm", res = 300)
ggplot(odonata.RI) +
  geom_smooth(aes(x=Gen, y=RI), formula = y~x, method = "lm", se = F) +
  geom_point(aes(x=Gen, y=RI)) +
...
# Plotting
png("../figures/04_Odonata_Barriers_1D.png", width = 24, height = 12, units = "cm", res = 300)
ggplot(odonata.RI) +
  geom_boxplot(aes(x=RI, y=Gen)) +
  geom_point(aes(x=RI, y=Gen)) +
