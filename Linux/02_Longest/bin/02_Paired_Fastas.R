...
for (i in groups) {
  ...
  temp = list.files(path = paste0("../results/",i,"/02_Species_Fastas/"), pattern = "*.fasta")
  # Editing names
  temp <- gsub(".fasta","",temp)
  # Reading all .fasta files and assigning them to their name
  for (n in temp) {
    print(n)
    fastas[[i]][[n]] <- read.fasta(paste0("../results/",i,"/02_Species_Fastas/",n,".fasta"), clean_name = F)
  }
...
for (i in groups) {
  ...
  fastas[[i]] <- list()
  dir.create(paste0("../results/",i,"/03_Paired_Fastas"), showWarnings = F)
  ...
  for (n in unique(species[[i]]$pair)) {
    ...
    if (nrow(fastas[[i]][[n]])==2) {
      ...
      phylotools::dat2fasta(fastas[[i]][[n]], outfile = paste0("../results/",i,"/03_Paired_Fastas/",fastas[[i]][[n]]$seq.name[1], "_X_", fastas[[i]][[n]]$seq.name[2],".fasta"))
    }
  }
}
