...
species <- list()
for (i in groups) {
  species[[i]] <- read.delim(paste0("../data/01_raw/",i,"_Barriers.tsv"), header = T)
  # Keeping only the species columns
  species[[i]] <- species[[i]][,1:2]
  colnames(species[[i]]) <- c("Sp1","Sp2")
  # Replacing " " by "_"
...
for (i in groups) {
  ...
  temp = list.files(path = paste0("../data/",i,"/04_Consensus/"), pattern = "*.fasta")
  # Editing names
  temp <- gsub(".fasta","",temp)
  # Reading all .fasta files and assigning them to their name
  for (n in temp) {
    print(n)
    fastas[[i]][[n]] <- read.fasta(paste0("../data/",i,"/04_Consensus/",n,".fasta"), clean_name = F)
  }
...
for (i in groups) {
  ...
  fastas[[i]] <- list()
  dir.create(paste0("../data/",i,"/05_Paired_Fastas"), showWarnings = F)
  ...
  for (n in unique(species[[i]]$pair)) {
    ...
    if (nrow(fastas[[i]][[n]])==2) {
      ...
      phylotools::dat2fasta(fastas[[i]][[n]], outfile = paste0("../data/",i,"/05_Paired_Fastas/",fastas[[i]][[n]]$seq.name[1], "_X_", fastas[[i]][[n]]$seq.name[2],".fasta"))
    }
  }
}
