...
species <- list()
for (i in groups) {
  species[[i]] <- read.table(paste0("../data/",i,"/01_BOLD/",i,"_species.txt"), sep = ",", header = F, col.names = "Species")
}

# Reading fasta files files per order
for (i in groups) {
  fastas[[i]] <- read.fasta(paste0("../data/",i,"/01_BOLD/",i,".fasta"), clean_name = F)
  # Filtering for COI
  ...
  write.table(fastas[[i]], paste0("../data/",i,"/01_BOLD/",i,"_info.tsv"), sep = "\t", quote = F, row.names = F)
  # Looking for missing species
  missings <- setdiff(species[[i]]$Species, sub("_"," ",fastas[[i]]$Species))
  # Writing missing species
  write(missings, paste0("../data/",i,"/01_BOLD/",i,"_missings.txt"), ncolumns = 1)
  # Splitting per species
  dir.create(paste0("../data/",i,"/02_Species_Fastas"), showWarnings = F)
  species[[i]] <- list()
  for (n in unique(fastas[[i]]$Species)) {
    ...
    phylotools::dat2fasta(species[[i]][[n]], outfile = paste0("../data/",i,"/02_Species_Fastas/",n,".fasta"))
  }
}
